...
def make_openai_response(request_id: str, model: str, content: str,
                         completion_id: str, message_id: str,
    ...
    async def event_generator():
        try:
            ...
            if not pending or not pending.reply:
                yield make_openai_chunk(
                    request_id, model, "⚠️ 人工回复为空或请求已过期",
            ...
            reply = pending.reply
            words = reply  # 直接返回全部内容
            yield make_openai_chunk(
                request_id, model, words,
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {"request": request, "pending_requests": items},
    )
...
if __name__ == "__main__":
    ...
    port = find_free_port()
    print(f"🚀 OpenAI Human Proxy 启动中...")
